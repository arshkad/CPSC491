...
from colorpalette import determine_season, get_season_description
...
def analyze_colors():
    ...
    try:
        ...
        print(f"Parsed RGB - Skin: {skin_rgb}, Hair: {hair_rgb}, Eyes: {eye_rgb}")
        
        # Analyze
        season = determine_season(skin_rgb, hair_rgb, eye_rgb)
        undertone = analyze_undertone(skin_rgb)
        
        print(f"Results - Season: {season}, Undertone: {undertone}")
...
@app.route('/health', methods=['GET'])
def health():
    return jsonify({"status": "running", "port": 5001})

if __name__ == '__main__':
    print("\n" + "="*50)
    print("ColorMe Starting on http://localhost:5001")
    print("="*50 + "\n")
    app.run(debug=True, port=5001, host='0.0.0.0')